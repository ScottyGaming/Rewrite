...
class MusicPlayer:
    ...
    async def player_loop(self):
        ...
        while not self.bot.is_closed():
            ...
            self.np = await self._channel.send(f'**Playing a song:** `{source.title}` signalled by '
                                               f'`{source.requester}`')
...
class Music(commands.Cog):
    ...
    async def connect_(self, ctx, *, channel: discord.VoiceChannel=None):
        ...
        if not channel:
            try:
                ...
                raise InvalidVoiceChannel('Please use this cmd in a VC!')
    ...
    async def lyrics(self, ctx, *, arg):
        ...
        try:
            ...
            embed = discord.Embed(title = f":musical_note: Lyrics | {title} | {author}", description = lyrics if len(lyrics) <= 2048 else lyrics[:2048], timestamp=ctx.message.created_at,colour = discord.Color.dark_blue())
            await ctx.send(embed = embed)
            embed = discord.Embed(title = f":musical_note: Lyrics | {title} | {author}", description = lyrics[2048:], timestamp=ctx.message.created_at,colour = discord.Color.dark_blue())
            await ctx.send(embed = embed)
        except Exception as e:
            await ctx.send(":information_source: Sorry, I can't found the lyrics for the song you mentioned.")
            print("Error occured | "+str(e) + " | "+str(data))
        except Exception as e:
            await ctx.send(f":no: Seems like an error has occurred: ```{e}```")
    ...
    async def pause_(self, ctx):
        ...
        if not vc or not vc.is_playing():
            return await ctx.send('Bot is Idle', delete_after=20)
        elif vc.is_paused():
            return
    ...
    async def queue_info(self, ctx):
        ...
        if not vc or not vc.is_connected():
            return await ctx.send('Bot not connected to VC!', delete_after=20)
    ...
    async def now_playing_(self, ctx):
        ...
        if not player.current:
            return await ctx.send('Bot is Idle!')
    ...
    async def change_volume(self, ctx, *, vol: float):
        ...
        if not 0 < vol < 101:
            return await ctx.send('Please enter volume between 1 and 100.')
